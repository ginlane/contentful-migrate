...
const initializeStoreStates = async (accessToken, spaceId, environmentId, clear) => {
  if (typeof cachedState !== 'undefined' && !clear) {
    return cachedState
  }
...
const createStoreFactory = async ({
  accessToken, spaceId, environmentId, dryRun, clear
}) => {
  defaultSpaceLocale = defaultSpaceLocale || await getDefaultLocale(accessToken, spaceId, environmentId)
  await initializeStoreStates(accessToken, spaceId, environmentId, clear)
